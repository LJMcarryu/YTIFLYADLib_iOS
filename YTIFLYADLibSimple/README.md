...
本工程演示 `YTIFLYADLib 6.3.1` 的开屏和插屏广告接入，两种格式均可展示图片或视频素材。Demo 不包含 Banner、激励视频或自渲染信息流入口。

`Podfile` 已固定到已发布的 `6.3.1` tag；同版本 Release、匿名下载和公开消费验证均已完成。候选/正式 CI 会将精确的合并 zip 解压为本地 Pod，实际构建该 Demo。
...
```ruby
pod 'YTIFLYADLib', :podspec => 'https://raw.githubusercontent.com/LJMcarryu/YTIFLYADLib_iOS/6.3.1/YTIFLYADLib.podspec'
```
