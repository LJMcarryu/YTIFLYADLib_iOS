...
当前正式版本：[`6.3.5`](https://github.com/LJMcarryu/YTIFLYADLib_iOS/releases/tag/6.3.5)。生产项目请固定到具体版本，不要依赖 `main` 分支。
...
选择版本 `6.3.5` 和产品 `YTIFLYADLib`。资源 target 会自动投递 `YTAdvSDK.bundle`；在 App target 的 `Other Linker Flags` 添加：
...
从 [Release 6.3.5](https://github.com/LJMcarryu/YTIFLYADLib_iOS/releases/tag/6.3.5) 下载 `YTIFLYADLib-6.3.5.zip`：
...
| 问题 | 处理方式 |
| --- | --- |
| 找不到 Banner、Reward 或 NativeFeed 类 | 这些能力不在 YT 6.3.5 产物中，请勿从其他变体复制代码。 |
